...
@section('title', 'Data Kategori Koleksi')
...
<div class="max-w-7xl mx-auto">
    ...
    <div class="bg-white rounded-[2.5rem] p-10 shadow-sm mb-10 border-b-8 border-[#c65c6a] flex flex-col lg:flex-row justify-between items-center gap-8 transition-all hover:shadow-md">
        <div>
            ...
            <h2 class="text-4xl font-black text-gray-800 tracking-tighter uppercase italic">Data <span class="text-[#c65c6a]">Kategori</span></h2>
        </div>

        <div class="flex flex-col md:flex-row items-center gap-5 w-full lg:w-auto">
            {{-- Form Search --}}
            <form action="{{ route('admin.category.index') }}" method="GET" class="relative group w-full md:w-80" id="searchForm">
            ...
            <a href="{{ route('admin.category.create') }}" class="w-full md:w-auto bg-[#c65c6a] hover:bg-gray-800 text-white px-10 py-4 rounded-2xl font-black shadow-xl shadow-[#c65c6a]/20 transition-all active:scale-95 flex items-center justify-center gap-3">
                ...
                <span class="tracking-widest text-[11px] uppercase whitespace-nowrap">Tambah Kategori</span>
            </a>
        </div>
    </div>
                    ...
                    <tr class="bg-white hover:bg-gray-50 hover:shadow-xl hover:shadow-gray-100 hover:scale-[1.01] transition-all duration-300 group cursor-default">
                        ...
                        <td class="px-10 py-6">
                            <div class="flex items-center gap-3">
                                ...
                                <span class="text-sm font-black text-gray-800 tracking-tight group-hover:text-[#c65c6a] transition-colors uppercase">
                                    {{ $cat->name }}
                                </span>
                            </div>
                        </td>
                        <td class="px-10 py-6">
                            <div class="flex justify-center gap-3">
                                ...
                                <form action="{{ route('admin.category.destroy', $cat->id) }}" method="POST" onsubmit="return confirm('Hapus kategori ini?')">
                                    @csrf @method('DELETE')
                    ...
                    <tr>
                        <td colspan="3" class="p-32 text-center">
                            <div class="opacity-20 flex flex-col items-center">
                                <i class="fas fa-folder-open text-7xl mb-6 text-gray-300"></i>
                                <p class="text-[11px] font-black uppercase tracking-[0.4em]">Belum Ada Data</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
